 ...
 * Tries Gemini first (human-like, legally enriched) and falls back to
 * the rule-based template when quota is exhausted.
 */

const { callGemini } = require('./geminiClient');
const { KEYS }       = require('./config');

// ── Per-category RTI information requests ─────────────────────────
...
// ── Rule-based template (no API needed) ──────────────────────────
...
${data.geoLat ? `The specific location of the issue is at coordinates: Lat. ${parseFloat(data.geoLat).toFixed(6)}°N, Long. ${parseFloat(data.geoLng).toFixed(6)}°E.` : ''}
...
// ── AI-powered draft (Gemini) ─────────────────────────────────────
async function generateRTIDraft(data, ai) {
  // No keys → immediately use template
  if (!KEYS.length) {
    console.log('📝 RTI draft: no Gemini keys — using rule-based template');
...
Write ONLY the RTI application text. No introduction, no explanation, no commentary before or after. Start directly with "APPLICATION UNDER THE RIGHT TO INFORMATION ACT, 2005".`;
  ...
  if (draft) {
    console.log(`📝 RTI draft written by Gemini (${draft.length} chars)`);
    return draft;
  }
